...
import { Star, Trophy, Users, TrendingUp, Search, ChevronRight, Gift, Award, History, Zap, Ticket, Clock, CheckCircle, XCircle, Printer, Download, RefreshCw } from "lucide-react";
...
const TYPE_LABELS: Record<string, { label: string; color: string }> = {
  ...
  tier_down: { label: "Nivel reiniciado", color: "text-slate-600 bg-slate-50 border-slate-200" },
...
export default function Loyalty() {
  ...
  const syncMutation = useMutation({
    mutationFn: () => apiFetch("/api/loyalty/sync", { method: "POST" }),
    onSuccess: (data: any) => {
      queryClient.invalidateQueries({ queryKey: ["/api/loyalty/leaderboard"] });
      Toast.fire({
        icon: "success",
        title: data?.processed > 0
          ? `Sincronización completada: ${data.processed} venta${data.processed > 1 ? "s procesadas" : " procesada"}`
          : "Clientes y compras sincronizados con el programa de fidelización",
      });
    },
    onError: (err: any) => {
      Toast.fire({ icon: "error", title: err.message || "Error al sincronizar puntos" });
    },
  });
  ...
  const totalRedeemed = (members ?? []).reduce((s, m) => s + m.lifetimeRedeemed, 0);
  const totalAvailable = (members ?? []).reduce((s, m) => s + m.points, 0);
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <Star className="w-6 h-6 text-amber-500" /> Programa de Fidelización
        </h1>
        <p className="text-muted-foreground text-sm mt-1">
          {settings.pointsPerUnit} pt{settings.pointsPerUnit !== 1 ? "s" : ""} por {currencySymbol}1 · 100 pts = {formatCurrency(100 * Number(settings.pointsRedemptionRate), currencySymbol)} · Niveles: 🥉×1 · 🥈×1.5 (1k pts) · 🥇×2 (5k pts)
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          variant="outline"
          onClick={() => setLocation(`/loyalty/${members?.[0]?.customerId ?? 0}/summary`)}
        ...
        <Button
          variant="outline"
          onClick={() => queryClient.invalidateQueries({ queryKey: ["/api/loyalty"] })}
          className="gap-2"
        >
        ...
        <Button
          ...
          className="gap-2 text-amber-700 hover:text-amber-800 border-amber-300 hover:bg-amber-50"
        >
          <RefreshCw className={`w-4 h-4 ${syncMutation.isPending ? "animate-spin" : ""}`} />
          {syncMutation.isPending ? "Sincronizando..." : "Sincronizar Ventas"}
        </Button>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {(Object.entries(TIERS) as [TierName, typeof TIERS.bronze][]).map(([key, t]) => {
          ...
          return (
            <button
              ...
              className={`rounded-2xl border-2 p-4 text-left transition-all ${
                tierFilter === key ? `${t.border} ${t.bg} shadow-md` : "border-border bg-white hover:border-border/80 hover:shadow-sm"
              }`}
            >
              <div className="flex items-center gap-2 mb-2">
                <span className="text-2xl">{t.emoji}</span>
                <span className={`font-bold text-sm ${tierFilter === key ? t.text : ""}`}>{t.label}</span>
              ...
              <p className="text-[11px] text-muted-foreground">
                {key === "bronze" ? "0 – 999 pts" : key === "silver" ? "1,000 – 4,999 pts" : "5,000+ pts"}
              </p>
              <p className={`text-xs font-bold mt-1 ${t.text}`}>×{t.multiplier} multiplicador</p>
            </button>
          );
        })}
      </div>
        ...
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold">{totalEarned.toLocaleString("es")}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-orange-700">{totalRedeemed.toLocaleString("es")}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] text-muted-foreground">Puntos Disponibles</p>
              <p className="text-xl font-bold text-emerald-700">{totalAvailable.toLocaleString("es")}</p>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            <div>
              <CardTitle className="text-base flex items-center gap-2">
                <Trophy className="w-4 h-4 text-amber-500" /> Ranking de Clientes
                {tierFilter !== "all" && (
                  <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full border ${TIERS[tierFilter as TierName].bg} ${TIERS[tierFilter as TierName].border} ${TIERS[tierFilter as TierName].text}`}>
                    {TIERS[tierFilter as TierName].emoji} {TIERS[tierFilter as TierName].label}
                  </span>
                )}
              </CardTitle>
            </div>
          ...
          {!isLoading && filtered.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm font-medium">
                {tierFilter !== "all" ? `Sin clientes en nivel ${TIERS[tierFilter as TierName]?.label}` : "Sin miembros aún"}
              </p>
            </div>
          )}
          {filtered.length > 0 && (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border/50 bg-muted/30">
                    <th className="text-left px-5 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wide w-10">#</th>
                    <th className="text-left px-4 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">Cliente / Nivel</th>
                    <th className="text-right px-4 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">Ganados</th>
                    <th className="text-right px-4 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wide hidden md:table-cell">Canjeados</th>
                    <th className="text-right px-4 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">Disponibles</th>
                    <th className="text-right px-4 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wide hidden lg:table-cell">Desc. disponible</th>
                    <th className="text-right px-4 py-3 text-[11px] font-semibold text-muted-foreground uppercase tracking-wide hidden xl:table-cell">Progreso</th>
                    <th className="px-4 py-3"></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border/30">
                  {filtered.map((m, idx) => {
                    const t = TIERS[m.tier] ?? TIERS.bronze;
                    return (
                      <tr key={m.customerId} className="hover:bg-muted/20 transition-colors">
                        <td className="px-5 py-3 text-muted-foreground font-medium text-center">
                          {idx === 0 ? "🥇" : idx === 1 ? "🥈" : idx === 2 ? "🥉" : <span className="text-xs">{idx + 1}</span>}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2.5">
                            <div className={`w-9 h-9 rounded-full border-2 flex items-center justify-center text-sm font-bold shrink-0 ${t.border} ${t.bg} ${t.text}`}>
                              ...
                              <div className="flex items-center gap-1.5 mt-0.5">
                                ...
                                {m.tierMultiplier > 1 && (
                                  <span className="text-[9px] text-muted-foreground flex items-center gap-0.5">
                                    <Zap className="w-2.5 h-2.5" />×{m.tierMultiplier}
                                  </span>
                                )}
                              </div>
                            </div>
                        ...
                        <td className="px-4 py-3 text-right">
                          <Badge variant="outline" className="font-bold text-primary border-primary/30 bg-primary/5">
                            <Star className="w-3 h-3 mr-1 text-amber-500" />{m.lifetimeEarned.toLocaleString("es")}
                          </Badge>
                        </td>
                        <td className="px-4 py-3 text-right hidden md:table-cell">
                          <span className={`text-xs font-semibold ${m.lifetimeRedeemed > 0 ? "text-orange-600" : "text-muted-foreground"}`}>
                            {m.lifetimeRedeemed.toLocaleString("es")} pts
                          </span>
                        </td>
                        <td className="px-4 py-3 text-right">
                          <span className={`font-bold ${m.points > 0 ? "text-amber-900" : "text-muted-foreground"}`}>
                            {m.points.toLocaleString("es")} pts
                          </span>
                        </td>
                        <td className="px-4 py-3 text-right font-semibold hidden lg:table-cell">
                          <span className={m.discountValue > 0 ? "text-emerald-700" : "text-muted-foreground"}>
                            {formatCurrency(m.discountValue, currencySymbol)}
                          </span>
                        </td>
                        <td className="px-4 py-3 hidden xl:table-cell w-48">
                          {m.nextTier ? (
                            <div className="space-y-1">
                              <div className="flex justify-between text-[10px] text-muted-foreground">
                                <span>{m.progressToNext}%</span>
                                <span>{m.pointsToNext.toLocaleString("es")} para {m.nextTier.label}</span>
                              </div>
                              <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                                <div className={`h-full rounded-full ${t.bar}`} style={{ width: `${m.progressToNext}%` }} />
                              </div>
                            </div>
                          ) : (
                            <span className={`text-[11px] font-semibold ${t.text}`}>✨ Nivel máximo</span>
                          )}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1 justify-end">
                            <Button size="sm" variant="ghost" className="h-7 px-2 text-xs text-violet-600 hover:bg-violet-50" onClick={() => setCouponsFor({ id: m.customerId, name: m.customerName })}>
                              <Ticket className="w-3.5 h-3.5 mr-1" /> Cupones
                            </Button>
                            <Button size="sm" variant="ghost" className="h-7 px-2 text-xs text-primary hover:bg-primary/10" onClick={() => setHistoryFor({ id: m.customerId, name: m.customerName })}>
                              <History className="w-3.5 h-3.5 mr-1" /> Historial
                            </Button>
                            <Button size="sm" variant="ghost" className="h-7 px-2 text-xs hover:bg-muted" onClick={() => handleAdjust(m)}>
      ...
      {couponsFor && <CouponsModal customerId={couponsFor.id} customerName={couponsFor.name} onClose={() => setCouponsFor(null)} />}
      {historyFor && <HistoryPanel customerId={historyFor.id} customerName={historyFor.name} onClose={() => setHistoryFor(null)} />}
    </div>
  );
}
