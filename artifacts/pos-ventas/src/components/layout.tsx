    ...
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      >
        <div className="h-16 flex items-center px-6 border-b border-sidebar-border bg-sidebar-accent/50">
          <div className="w-8 h-8 rounded bg-primary flex items-center justify-center text-primary-foreground font-bold mr-3">
            P
          </div>
          <span className="text-xl font-bold text-sidebar-foreground">POS Ventas</span>
          <Button 
            variant="ghost" 
            size="icon" 
        ...
        <header className="h-16 flex items-center justify-between px-4 sm:px-6 bg-card border-b z-30">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-semibold hidden sm:block">
              {NAV_ITEMS.find(item => location.startsWith(item.href))?.label || "POS"}
            </h1>
          </div>
        </header>
